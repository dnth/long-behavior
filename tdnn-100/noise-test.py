...
tdnnclassifier = NetworkReader.readFrom('2sigmoid/TrainUntilConv.xml')
...
for testnumber in range(30):    
    ...
    x = tdnnclassifier.activate(LBallLift[start:start+100].flatten())
    predictedLBLLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RBallLift[start:start+100].flatten())
    predictedRBLLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LBallRoll[start:start+100].flatten())
    predictedLBRLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RBallRoll[start:start+100].flatten())
    predictedRBRLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LBallRollPlate[start:start+100].flatten())
    predictedLBRPLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RBallRollPlate[start:start+100].flatten())
    predictedRBRPLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LRopeway[start:start+100].flatten())
    predictedLRWLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RRopeway[start:start+100].flatten())
