...
LSTMClassificationNet = NetworkReader.readFrom('43sigmoid/TrainUntilConv.xml')
print 'Loaded RNN Trained Network!'
...
redundantSeqLen = 0
while (redundantSeqLen<=2000):
    redundantSeqLen += 100
    print "Redundant Seqlen:",redundantSeqLen
    predictedLBLLabels = []
    predictedRBLLabels = []
    predictedLBRLabels = []
    predictedRBRLabels = []
    ...
    for testnumber in range(30):    
        ...
        for i in timestep:
            x = LSTMClassificationNet.activate(LBallLift[i])
        for i in range(redundantSeqLen):
...
np.savetxt("AccuVsBehaviorLength.txt", twentylstmaccdata)