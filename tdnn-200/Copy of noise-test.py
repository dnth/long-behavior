...
sequenceStartIndex = range(160000,198200,2000)
offset = 200
accuracyOverall = []
for testnumber in range(30):    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LBL[start:start+2000])
    predictedLBLLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RBL[start:start+2000])
    predictedRBLLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LBR[start:start+2000])
    ...
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LBRP[start:start+2000])
    predictedLBRPLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RBRP[start:start+2000])
    predictedRBRPLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(LRW[start:start+2000])
    predictedLRWLabels.append(argmax(x))
    
    start = np.random.choice(sequenceStartIndex)
    x = tdnnclassifier.activate(RRW[start:start+2000])
