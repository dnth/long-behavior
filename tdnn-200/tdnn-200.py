...
trndata = ClassificationDataSet(2000,1, nb_classes=8)
tstdata = ClassificationDataSet(2000,1, nb_classes=8)
...
for i in range(0,120000,2000):
    trndata.addSample(LBL[i:i+2000], [0])
    trndata.addSample(RBL[i:i+2000], [1])
    trndata.addSample(LBR[i:i+2000], [2])
    trndata.addSample(RBR[i:i+2000], [3])
    trndata.addSample(LBRP[i:i+2000], [4])
    trndata.addSample(RBRP[i:i+2000], [5])
    trndata.addSample(LRW[i:i+2000], [6])
    trndata.addSample(RRW[i:i+2000], [7])
    
for i in range(120000,160000,2000):
    tstdata.addSample(LBL[i:i+2000], [0])
    tstdata.addSample(RBL[i:i+2000], [1])
    tstdata.addSample(LBR[i:i+2000], [2])
    tstdata.addSample(RBR[i:i+2000], [3])
    tstdata.addSample(LBRP[i:i+2000], [4])
    tstdata.addSample(RBRP[i:i+2000], [5])
    tstdata.addSample(LRW[i:i+2000], [6])
    tstdata.addSample(RRW[i:i+2000], [7])
...
TDNNClassificationNet = buildNetwork(trndata.indim,1,trndata.outdim, hiddenclass=SigmoidLayer, outclass=SoftmaxLayer) 
...
trnErrorPath='1sigmoid/trn_error'
tstErrorPath='1sigmoid/tst_error'
trnClassErrorPath='1sigmoid/trn_ClassAccu'
tstClassErrorPath='1sigmoid/tst_ClassAccu'
networkPath='1sigmoid/TrainUntilConv.xml'
figPath='1sigmoid/ErrorGraph'
...
#     print "********** Classification with 1sigmoid with RP- **********"   
...
# np.savetxt("1sigmoid/Trainingtime.txt", trainingTime)


####################
# Manual OFFLINE Test
####################        
TDNNClassificationNet = NetworkReader.readFrom('1sigmoid/TrainUntilConv.xml')
trainer = RPropMinusTrainer(TDNNClassificationNet, dataset=trndata, verbose=True, weightdecay=0.01)
print 'Loaded Trained Network!'
print TDNNClassificationNet.paramdim
   
# testingdata = ClassificationDataSet(2000,1, nb_classes=8)  
#         
# for i in range(160000,198200,2000): # if 200000 will go beyond array value. 19800 + 2000 = 200000
#     testingdata.addSample(LBL[i:i+2000], [0])
#     testingdata.addSample(RBL[i:i+2000], [1])
#     testingdata.addSample(LBR[i:i+2000], [2])
#     testingdata.addSample(RBR[i:i+2000], [3])
#     testingdata.addSample(LBRP[i:i+2000], [4])
#     testingdata.addSample(RBRP[i:i+2000], [5])
#     testingdata.addSample(LRW[i:i+2000], [6])
#     testingdata.addSample(RRW[i:i+2000], [7])
#       
...
# plt.plot(RBallLift[19000:19400])
# plt.show()
testdata = LBallRoll[19000:19200].flatten()
